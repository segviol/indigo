...
struct loop_info {
  ...
  std::vector<std::unique_ptr<mir::inst::Inst>> rewrite_jump(
      mir::inst::BasicBlk& blk, mir::inst::OpInst& inst) {
    auto val = blk.jump.cond_or_ret.value();
    auto t = get_times();
    auto res = copy_var(inst.dest);
    blk.inst.push_back(std::make_unique<mir::inst::OpInst>(
        res, cmp.second, init_var.second, mir::inst::Op::Sub));
    auto multimes = copy_var(inst.dest);
    blk.inst.push_back(std::make_unique<mir::inst::OpInst>(
        multimes, inst.lhs, 4, mir::inst::Op::Mul));
    auto cond = copy_var(val);
    blk.inst.push_back(std::make_unique<mir::inst::OpInst>(cond, multimes, 4,
                                                           mir::inst::Op::Mul));
  }
};
