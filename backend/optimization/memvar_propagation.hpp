  ...
  void optimize_func(mir::inst::MirFunction& func) {
    ...
    for (bit = func.basic_blks.begin(); bit != func.basic_blks.end(); bit++) {
      ...
      for (int j = 0; j < call_index.size(); j++) {
        ...
        std::map<mir::inst::VarId, mir::inst::VarId> store;
        std::map<mir::inst::VarId, mir::inst::VarId> load;
        std::map<mir::inst::VarId, mir::inst::VarId>::iterator it;
        index = 0;
        for (auto& inst : bb.inst) {
          if (index >= call_index[j] && index < upper_bound) {
            ...
            if (auto x = dynamic_cast<mir::inst::StoreInst*>(&i)) {
              if (x->val.index() == 1) {
                it = store.find(std::get<1>(x->val));
                if (it == store.end()) {
                  store.insert(
                      std::map<mir::inst::VarId, mir::inst::VarId>::value_type(
                          x->dest, std::get<1>(x->val)));
                } else {
                  store[x->dest] = std::get<1>(x->val);
                }
              }
              ...
              if (x->src.index() == 1) {
                ...
                if (it != store.end()) {
                  std::map<mir::inst::VarId, mir::inst::VarId>::iterator iter;
                  iter = load.find(x->dest);
                  if (iter == load.end()) {
                    load.insert(
                        std::map<mir::inst::VarId,
                                 mir::inst::VarId>::value_type(x->dest,
                                                               it->second));
                  } else {
            ...
            if (auto x = dynamic_cast<mir::inst::AssignInst*>(&i)) {
              ...
              for (int j = 0; j < x->params.size(); j++) {
                if (x->params[j].index() == 1) {
                  std::map<mir::inst::VarId, mir::inst::VarId>::iterator lit =
                      load.find(std::get<1>(x->params[j]));
                  if (lit != load.end()) {
