...
class FakeGenerator {
 public:
  ...
  void fakeMirGenerator1() {
    ...
    _package->functions.at(0).variables[2] =
        Variable(SharedTyPtr(new IntTy()), true);
    _package->functions.at(0).variables[3] =
        Variable(SharedTyPtr(new IntTy()), true);
    _package->functions.at(0).variables[4] =
        Variable(SharedTyPtr(new PtrTy(SharedTyPtr(new IntTy()))), false, true);
    _package->functions.at(0).variables[5] =
        Variable(SharedTyPtr(new IntTy()), false, true);
    _package->functions.at(0).variables[6] =
        Variable(SharedTyPtr(new PtrTy(SharedTyPtr(new IntTy()))), false, true);

    _package->functions.at(0).basic_blks.insert({0, mir::inst::BasicBlk(0)});
    _package->functions.at(0).basic_blks.insert({1, mir::inst::BasicBlk(1)});
    _package->functions.at(0).basic_blks.insert({2, mir::inst::BasicBlk(2)});
    _package->functions.at(0).basic_blks.insert({3, mir::inst::BasicBlk(3)});

    _package->functions.at(0).basic_blks.at(0).jump = JumpInstruction(
        JumpInstructionKind::Br, 3, -3, nullopt, JumpKind::Undefined);
    _package->functions.at(0).basic_blks.at(0).inst.push_back(
    ...
    for (i = 0; i < 10; i++) {
      ...
      if (i < 9) {
        _package->functions.at(0).basic_blks.at(0).inst.push_back(move(
    ...
    _package->functions.at(0).basic_blks.at(3).id = 3;
    _package->functions.at(0).basic_blks.at(3).jump = std::move(JumpInstruction(
        JumpInstructionKind::BrCond, 1, 2, VarId(5), JumpKind::Loop));
    _package->functions.at(0).basic_blks.at(3).preceding.insert(0);
    _package->functions.at(0).basic_blks.at(3).preceding.insert(1);
    value1.emplace<0>(9);
    value2.emplace<1>(VarId(2));
    _package->functions.at(0).basic_blks.at(3).inst.push_back(
    ...
    _package->functions.at(0).basic_blks.at(1).id = 1;
    _package->functions.at(0).basic_blks.at(1).jump = move(JumpInstruction(
        JumpInstructionKind::Br, 3, -1, nullopt, JumpKind::Loop));
    _package->functions.at(0).basic_blks.at(1).preceding.insert(3);
    _package->functions.at(0).basic_blks.at(1).inst.push_back(
        move(unique_ptr<Inst>(new RefInst(VarId(4), VarId(1)))));
    value1.emplace<1>(VarId(2));
    _package->functions.at(0).basic_blks.at(1).inst.push_back(
    ...
    _package->functions.at(0).basic_blks.at(2).id = 2;
    _package->functions.at(0).basic_blks.at(2).jump = move(JumpInstruction(
        JumpInstructionKind::Return, -1, -1, nullopt, JumpKind::Undefined));
    _package->functions.at(0).basic_blks.at(2).preceding.insert(3);
  }
};
}  // namespace front::fake
