 ...
 * the appearance of error is that two symbols are pushed with same name and layerNum
 */
void SymbolTable::push_symbol(SharedSyPtr symbol)
{
    if (symbol->getLayerNum() != find_least_layer_symbol(symbol->getName())->getLayerNum())
    {
...
/*
 * this method will del the Symbol object on the top of stack which was allocated by new in push_symbol method
 */
...
/*
 * this method will pop the input layer`s all symbols only when they are on the top of stack
 */
