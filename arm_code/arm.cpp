...
/// Generate a number as Operand2. If not possible, insert code to put number
/// into a register generated by function make_reg and use the register as
/// result.
template <typename T, typename MakeReg>
Operand2 make_number(MakeReg make_reg, int32_t num,
                     std::insert_iterator<T> inst) {
  if (is_valid_immediate(num)) {
    return Operand2((int32_t)num);
  } else if ((~num) <= 0xffff) {
    Reg r = make_reg();
    inst.insert(std::make_unique<Arith2Inst>(arm::OpCode::Mvn, r, ~num));
    return RegisterOperand(r);
  } else {
    Reg r = make_reg();
    inst.insert(
        std::make_unique<Arith2Inst>(arm::OpCode::Mov, r, num & 0xffff));
    if (num > 0xffff) {
      inst.insert(
          std::make_unique<Arith2Inst>(arm::OpCode::MovT, r, num >> 16));
    }
    return RegisterOperand(r);
  }
}
